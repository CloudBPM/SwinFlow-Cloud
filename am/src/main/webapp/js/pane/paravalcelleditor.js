...
(function($, window, document, undefined) {
	var pluginName = "parameterValueCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};
	...
	TextCellEditor.prototype.changeValue = function(tag, entity, oldvalue,
			headbody) {
		var loc = entity.findParamPosition($(tag).prev('td').prev('td').text(),
				headbody);
		var newvalue = $(tag).children("input").val();
		if (newvalue != oldvalue) {
			map[entity.id].stack.execute(new AMParaValueChangedCmd(entity,
					oldvalue, newvalue,
