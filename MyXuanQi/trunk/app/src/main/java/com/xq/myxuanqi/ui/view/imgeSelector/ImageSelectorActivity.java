...
import com.xq.myxuanqi.viewModel.ImageSelectorViewModel;
...
public class ImageSelectorActivity extends BaseActivity {
    ...
    private ImageSelectorViewModel mImageSelectorViewModel;
    ...
    public static void openActivity(Activity activity, int requestCode,
        ...
        intent.putExtras(dataPackages(isSingle, isViewImage, useCamera, maxSelectCount, selected,requestCode));
    ...
    public static Bundle dataPackages(boolean isSingle, boolean isViewImage, boolean useCamera,
                                      int maxSelectCount, ArrayList<String> selected,int requestCode) {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mImageSelectorViewModel = ViewModelProviders.of(this).get(ImageSelectorViewModel.class);
    ...
    private void setResult(ArrayList<String> images, boolean isCameraImage) {
        if (mRequestCode==3){   //意见反馈上传图片
            File file = new File(images.get(0));
            if (file.exists()&&file.isFile()){
                String uid = mLogin.getUser().getId();
                mImageSelectorViewModel.uploadImage(file,uid,file.getName(),"0","image/");
                mImageSelectorViewModel.setUploadImageListener(new ImageSelectorViewModel.UploadImageListener() {
                    @Override
                    public void onUploadImageListener(int i) {
                        switch (i){
                            case 0:
                                ...
                                intent.putExtra("path", mImageSelectorViewModel.getMutableLiveData().getValue().getPath());
                            ...
                            case -1:
                                ...
                                Toast.makeText(ImageSelectorActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                                break;
                            case -2:
                                if (isCameraImage){
                                    file.delete();
                                }
                                Toast.makeText(ImageSelectorActivity.this, "网络连接超时", Toast.LENGTH_SHORT).show();
                                break;
                            case -3:
                                if (isCameraImage){
                                    file.delete();
                                }
                                Toast.makeText(ImageSelectorActivity.this, "网络连接失败", Toast.LENGTH_SHORT).show();
                                break;
                        }
                        finish();
                    }
...
//            Toast.makeText(this, "没有图片", Toast.LENGTH_LONG).show();
    ...
    private void openCamera() {
        ...
        if (captureIntent.resolveActivity(getPackageManager()) != null) {
            ...
            if (photoFile != null) {
                mPhotoPath = photoFile.toString();
                //通过FileProvider创建一个content类型的Uri
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {  //如果是7.0以上，使用FileProvider，否则会报错
                    Uri photoUri = FileProvider.getUriForFile(this, "com.xq.myxuanqi.fileprovider", photoFile);
                    captureIntent.putExtra(MediaStore.EXTRA_OUTPUT, photoUri);//设置拍照后图片保存的位置
                } else {
                    captureIntent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(photoFile)); //设置拍照后图片保存的位置
                }
    ...
    private File createImageFile() throws IOException {
        ...
        File file = new File(s+"/Pictures");
        if (!file.exists()) {
            file.mkdir();
        }
        File tempFile = new File(file, imageFileName);
        return tempFile;
    }
