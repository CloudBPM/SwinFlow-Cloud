...
/**
 * jedis工具类，使用时项目classPath下必须有sysconfig.properties文件
 * 配置内容:
...
public class JedisUtil {
    ...
    private JedisUtil() {
        if (jedisPool == null) {
            ...
            String host= SystemConfig.getProp("redis_host");
            int port=Integer.parseInt(SystemConfig.getProp("redis_port"));
            String pwd= SystemConfig.getProp("redis_password");
            String timeout= SystemConfig.getProp("redis_timeout");
            int tout = Integer.parseInt(timeout);
            jedisPool= new JedisPool(config,host,port,tout,pwd);
            /*
