...
function CurrencyInput() {
    ...
    this.append = null;
    this.classtypename = "CurrencyInput";
...
CurrencyInput.prototype = new UIComponent();

// for previewing
CurrencyInput.prototype.clone = function () {
    var t = new CurrencyInput();
    t.id = this.id;
...
// for previewing
CurrencyInput.prototype.cloneRules = function (owner, old) {
    this.f = owner;
...
// for previewing
CurrencyInput.prototype.toDomForHTML = function(parent) {
...
CurrencyInput.prototype.toDom = function (parent) {
    this.toDomforFormGroup(parent);
    this.updateDom();
};

CurrencyInput.prototype.updateDom = function () {
...
CurrencyInput.prototype.updateValueforRuntime = function () {
    var o = document.getElementById("currency" + this.id);
    o.value = this.initValue;
};

CurrencyInput.prototype.editable = function (e) {
    var o = document.getElementById("currency" + this.id);
    if (e == "0") {
        o.readOnly = false;
    } else {
...
CurrencyInput.prototype.show = function (d) {
    if (this.f != undefined) {
...
CurrencyInput.prototype.enable = function (e) {
    if (this.f != undefined) {
        var o = this.f.seekObjectByID(this.id);
        var i = o.input;
        if (e == "1") {
...
CurrencyInput.prototype.toDomforFormGroup = function (parent) {
    this.dom = document.createElement("DIV");
    parent.appendChild(this.dom);
    this.dom.id = this.id;
...
CurrencyInput.prototype.toRealValue = function () {
    var input = document.getElementById("currency" + this.id);
...
CurrencyInput.prototype.handleEvent = function (e) {
    switch (e.type) {
...
CurrencyInput.prototype.doClick = function (evt) {
    if (this.evn == 0) {
...
CurrencyInput.prototype.doFocus = function (evt) {
    this.doHandle(evt, 4);
...
CurrencyInput.prototype.doBlur = function (evt) {
    this.doHandle(evt, 5);
...
CurrencyInput.prototype.doChange = function (evt) {
    if (this.evn == 0) {
        this.doHandle(evt, 2);
        var e = document.getElementById("currency" + this.id);
        if (evt.target == e) {
...
CurrencyInput.prototype.doSelect = function (evt) {
    this.doHandle(evt, 3);
    Utils.stopBubble(evt);
};

CurrencyInput.prototype.doInput = function (evt) {
    this.doHandle(evt, 1);
    Utils.stopBubble(evt);
};

CurrencyInput.prototype.doKeypress = function (evt) {
    var o = document.getElementById("currency" + this.id);
    Utils.blockNonNumbers(o, evt, true, false);
    Utils.stopBubble(evt);
};

CurrencyInput.prototype.doPaste = function (evt) {
    Utils.stopDefaultEvent(evt);
};

CurrencyInput.prototype.doDragEnter = function (evt) {
    Utils.stopDefaultEvent(evt);
};

CurrencyInput.prototype.doCompositionStart = function (evt) {
    this.cpl = true;
};

CurrencyInput.prototype.doCompositionEnd = function (evt) {
    this.cpl = false;
};

CurrencyInput.prototype.message = function (evt, cmd, msg) {
...
CurrencyInput.prototype.act = function (evt, cmd, o, e, val) {
    if (cmd == 1) {
        o.show("0"); // hidden
    } else if (cmd == 2) {
        o.show("1"); // show
...
CurrencyInput.prototype.fCmd = function (evt, r, e) {
...
CurrencyInput.prototype.tCmd = function (evt, r, e) {
...
CurrencyInput.prototype.doHandle = function (evt, cmd) {
    try {
        var e = document.getElementById("currency" + this.id);
...
CurrencyInput.prototype.doDragStart = function (evt) {
    if (this.evn == 0) {
        if (map[this.currOwner] != null
            && map[this.currOwner].currObject instanceof Form) {
...
CurrencyInput.prototype.doDrag = function (evt) {
    Utils.stopBubble(evt);
};

CurrencyInput.prototype.doDragEnd = function (evt) {
    if (this.evn == 0) {
        if (map[this.currOwner] != null
            && map[this.currOwner].currObject instanceof Form) {
...
CurrencyInput.prototype.parseFromJSON = function (json, evn) {
    this.id = json.id;
...
CurrencyInput.prototype.parseExpressions = function (owner) {
    for (var i = 0; i < this.rules.length; i++) {
        this.rules[i].parseExpressions(owner);
    }
};

CurrencyInput.prototype.toTree = function () {
    return {
        id: this.id,
        text: this.title,
        icon: "fa fa-file-text-o",
        data: "货币输入框|" + (this.initValue == null ? "" : this.initValue),
    }
};

CurrencyInput.prototype.fetchRuleByNum = function (number) {
...
CurrencyInput.prototype.toString = function () {
    return this.title == null ? "货币输入框" : this.title;
};

CurrencyInput.prototype.addRule = function (r) {
    this.rules.push(r);
};

CurrencyInput.prototype.insertRule = function (r, index) {
    this.rules.splice(index, 0, r);
};

CurrencyInput.prototype.removeRule = function (r) {
    for (var i = 0; i < this.rules.length; i++) {
        if (this.rules[i] == r) {
