...
function StaticList() {
	...
	this.listType = 0; // 0:list without number; 1:list with number;
	this.hidden = 0; // 0: visible; 1: invisible
	this.classtypename = "StaticList";
...
StaticList.prototype = new UIComponent();

// for previewing
StaticList.prototype.clone = function() {
	var t = new Paragraph();
	t.id = this.id;
...
// for previewing
StaticList.prototype.toDomForHTML = function(parent) {
...
StaticList.prototype.toDom = function(parent) {
	this.toDomforFormGroup(parent);
	this.updateDom();
};

StaticList.prototype.updateDom = function() {
	...
	if (this.listType == 0) {
		...
		p.appendChild(li);
		this.parseItems(li, this.content);
	} else {
...
StaticList.prototype.parseItems = function(parent, content) {
	var lines = content.split(/\n/);
	if (lines.length > 0) {
		for (var i = 0; i < lines.length; i++) {
			this.addItem(parent, lines[i]);
		}
	}
};

StaticList.prototype.addItem = function(parent, value) {
	var item = document.createElement("li");
	parent.appendChild(item)
	item.innerHTML = value;
};

StaticList.prototype.toDomforFormGroup = function(parent) {
	this.dom = document.createElement("DIV");
	parent.appendChild(this.dom);
	this.dom.id = this.id;
...
StaticList.prototype.handleEvent = function(e) {
	switch (e.type) {
...
StaticList.prototype.doClick = function(evt) {
	if (this.evn == 0) {
...
StaticList.prototype.doDrag = function(evt) {
	Utils.stopBubble(evt);
};

StaticList.prototype.doDragEnd = function(evt) {
	if (this.evn == 0) {
		if (map[this.currOwner] != null
			&& map[this.currOwner].currObject instanceof Form) {
...
StaticList.prototype.parseFromJSON = function(json, evn) {
	this.id = json.id;
...
StaticList.prototype.toTree = function() {
	return {
		id : this.id,
		text : this.title,
		icon : "glyphicon glyphicon-list",
		data : "静态列表|" + this.content,
	}
};
