...
(function ($, window, document, undefined) {
    var pluginName = "deletionConfirmInfoDialog";
    var defaults = {
        id: "",
        title: "",
    };
    ...
    ConfirmDialog.prototype.doClick = function (evt) {
        if (evt.target.id == "YESButton" + this.options.id) {
            this.doYesAction(evt);
        } else if (evt.target.id == "NOButton" + this.options.id) {
            this.doNoAction(evt);
        }
    };

    ConfirmDialog.prototype.doNoAction = function (evt) {
        Utils.stopBubble(evt);
        this.hide();
    };

    ConfirmDialog.prototype.doYesAction = function (evt) { // 确认删除文件
        Utils.stopBubble(evt);
        $("progressbar").show();
        this.hide();
        var that = this;
