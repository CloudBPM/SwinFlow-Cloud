...
function DecimalsInput() {
    ...
    this.append = null;
    this.classtypename = "DecimalsInput";
...
DecimalsInput.prototype = new UIComponent();

// for previewing
DecimalsInput.prototype.clone = function () {
    var t = new DecimalsInput();
    t.id = this.id;
...
// for previewing
DecimalsInput.prototype.cloneRules = function (owner, old) {
    this.f = owner;
...
// for previewing
DecimalsInput.prototype.toDomForHTML = function(parent) {
...
DecimalsInput.prototype.toDom = function (parent) {
    this.toDomforFormGroup(parent);
    this.updateDom();
};

DecimalsInput.prototype.updateDom = function () {
...
DecimalsInput.prototype.updateValueforRuntime = function () {
    var o = document.getElementById("decimals" + this.id);
    o.value = this.initValue;
};

DecimalsInput.prototype.editable = function (e) {
    var o = document.getElementById("decimals" + this.id);
    if (e == "0") {
        o.readOnly = false;
    } else {
...
DecimalsInput.prototype.show = function (d) {
    if (this.f != undefined) {
...
DecimalsInput.prototype.enable = function (e) {
    if (this.f != undefined) {
        var o = this.f.seekObjectByID(this.id);
        var i = o.input;
        if (e == "1") {
...
DecimalsInput.prototype.toDomforFormGroup = function (parent) {
    this.dom = document.createElement("DIV");
    parent.appendChild(this.dom);
    this.dom.id = this.id;
...
DecimalsInput.prototype.toRealValue = function () {
    var input = document.getElementById("decimals" + this.id);
...
DecimalsInput.prototype.handleEvent = function (e) {
    switch (e.type) {
...
DecimalsInput.prototype.doClick = function (evt) {
    if (this.evn == 0) {
...
DecimalsInput.prototype.doFocus = function (evt) {
    this.doHandle(evt, 4);
...
DecimalsInput.prototype.doBlur = function (evt) {
    this.doHandle(evt, 5);
...
DecimalsInput.prototype.doChange = function (evt) {
    if (this.evn == 0) {
        this.doHandle(evt, 2);
        var e = document.getElementById("decimals" + this.id);
        if (evt.target == e) {
...
DecimalsInput.prototype.doSelect = function (evt) {
    this.doHandle(evt, 3);
    Utils.stopBubble(evt);
};

DecimalsInput.prototype.doInput = function (evt) {
    this.doHandle(evt, 1);
    Utils.stopBubble(evt);
};

DecimalsInput.prototype.doKeypress = function (evt) {
    var o = document.getElementById("decimals" + this.id);
    Utils.blockNonNumbers(o, evt, true, true);
    Utils.stopBubble(evt);
};

DecimalsInput.prototype.doPaste = function (evt) {
    Utils.stopDefaultEvent(evt);
};

DecimalsInput.prototype.doDragEnter = function (evt) {
    Utils.stopDefaultEvent(evt);
};

DecimalsInput.prototype.doCompositionStart = function (evt) {
    this.cpl = true;
};

DecimalsInput.prototype.doCompositionEnd = function (evt) {
    this.cpl = false;
};

DecimalsInput.prototype.message = function (evt, cmd, msg) {
...
DecimalsInput.prototype.act = function (evt, cmd, o, e, val) {
    if (cmd == 1) {
        o.show("0"); // hidden
    } else if (cmd == 2) {
        o.show("1"); // show
...
DecimalsInput.prototype.fCmd = function (evt, r, e) {
...
DecimalsInput.prototype.tCmd = function (evt, r, e) {
...
DecimalsInput.prototype.doHandle = function (evt, cmd) {
    try {
        var e = document.getElementById("decimals" + this.id);
...
DecimalsInput.prototype.doDragStart = function (evt) {
    if (this.evn == 0) {
        if (map[this.currOwner] != null
            && map[this.currOwner].currObject instanceof Form) {
...
DecimalsInput.prototype.doDrag = function (evt) {
    Utils.stopBubble(evt);
};

DecimalsInput.prototype.doDragEnd = function (evt) {
    if (this.evn == 0) {
        if (map[this.currOwner] != null
            && map[this.currOwner].currObject instanceof Form) {
...
DecimalsInput.prototype.parseFromJSON = function (json, evn) {
    this.id = json.id;
...
DecimalsInput.prototype.parseExpressions = function (owner) {
    for (var i = 0; i < this.rules.length; i++) {
        this.rules[i].parseExpressions(owner);
    }
};

DecimalsInput.prototype.toTree = function () {
    return {
        id: this.id,
        text: this.title,
        icon: "fa fa-file-text-o",
        data: "小数输入框|" + (this.initValue == null ? "" : this.initValue),
    }
};

DecimalsInput.prototype.fetchRuleByNum = function (number) {
...
DecimalsInput.prototype.toString = function () {
    return this.title == null ? "小数输入框" : this.title;
};

DecimalsInput.prototype.addRule = function (r) {
    this.rules.push(r);
};

DecimalsInput.prototype.insertRule = function (r, index) {
    this.rules.splice(index, 0, r);
};

DecimalsInput.prototype.removeRule = function (r) {
    for (var i = 0; i < this.rules.length; i++) {
        if (this.rules[i] == r) {
