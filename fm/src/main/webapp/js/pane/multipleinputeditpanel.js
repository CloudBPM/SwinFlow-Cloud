...
(function($, window, document, undefined) {
	var pluginName = "multipleLineInputEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "mul-input-props-nav-tabs";
		...
		var that = this;
		$("#mul-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).multipleLineInputPropPane({
			id : "tab0",
		...
		this.multipleInputPropPane = plugin1.data("multipleLineInputPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).multipleLineInputAssociatedPane({
			id : "tab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.multipleInputAssociatedPane = plugin2
				.data("multipleLineInputAssociatedPane");
		// by default
		$('#mul-input-props-nav-tabs a[href="#tab' + this.currtabindex + '"]')
				.tab('show');
	...
	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.multipleInputAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.multipleInputAssociatedPane.removeRow(evt);
		}
	};
