...
(function($, window, document, undefined) {
	var pluginName = "datetimeInputPropPane";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	BasicPropPanel.prototype.init = function(options) {
		...
		label3.innerHTML = "日期/时间类型";

		var selDiv3 = document.createElement("div");
		selDiv3.className = "col-sm-10";
		group3.appendChild(selDiv3);

		this.dtTypeSelect = document.createElement("Select");
		selDiv3.appendChild(this.dtTypeSelect);
		this.dtTypeSelect.id = "default" + options.id;
		this.dtTypeSelect.name = "default" + options.id;
		this.dtTypeSelect.className = "form-control";
		this.dtTypeSelect.setAttribute("placeholder", "默认值为文本框中的初始值");
		this.dtTypeSelect.addEventListener("change", this, false);

		this.addOptions(this.dtTypeSelect, "日期时间", "DateTime", 0);
		this.addOptions(this.dtTypeSelect, "日期", "Date", 1);
		this.addOptions(this.dtTypeSelect, "时间", "Time", 2);
		...
		if (options.currowner instanceof ReleasedForm) {
			...
			this.dtTypeSelect.disabled = true;
		}
	...
	BasicPropPanel.prototype.loadData = function(entity) {
		...
		this.dtTypeSelect.value = this.entity.datatype;
	};
	...
	BasicPropPanel.prototype.doChange = function(evt) {
		if (evt.target == this.dtTypeSelect) {
			map[this.options.currowner.id].stack.execute(new FMValueChangedCmd(
					this.entity, "datatype", this.dtTypeSelect.value,
