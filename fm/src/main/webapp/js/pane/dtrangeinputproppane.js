...
(function ($, window, document, undefined) {
    var pluginName = "dtrangeInputPropPane";
    var defaults = {
        id: "",
        parent: "",
        entity: "",
    ...
    BasicPropPanel.prototype.init = function (options) {
        ...
        label3.innerHTML = "显示选项";

        var selDiv3 = document.createElement("div");
        selDiv3.className = "col-sm-10";
        group3.appendChild(selDiv3);

        this.check0 = this.createCheckBox(selDiv3, this.entity.id, "0", "天");
        this.check1 = this.createCheckBox(selDiv3, this.entity.id, "1", "小时");
        this.check2 = this.createCheckBox(selDiv3, this.entity.id, "2", "分钟");
        this.check3 = this.createCheckBox(selDiv3, this.entity.id, "3", "秒");
        ...
        if (options.currowner instanceof ReleasedForm) {
            ...
            this.check0.disabled = true;
            this.check1.disabled = true;
            this.check2.disabled = true;
            this.check3.disabled = true;
        }

    };

    BasicPropPanel.prototype.createCheckBox = function (p, id, value, title) {
        var selDiv = document.createElement("label");
        selDiv.className = "checkbox-inline";
        p.appendChild(selDiv);

        var checkbox = document.createElement("INPUT");
        selDiv.appendChild(checkbox);
        checkbox.type = "checkbox";
        checkbox.name = "opts" + id;
        checkbox.value = value;
        checkbox.addEventListener("click", this, false);
        selDiv.appendChild(document.createTextNode(title));
        return checkbox;
    };
    ...
    BasicPropPanel.prototype.loadData = function (entity) {
        ...
        if (this.entity.opts[0] == 0) {
            this.check0.checked = false;
        } else {
            this.check0.checked = true;
        }
        if (this.entity.opts[1] == 0) {
            this.check1.checked = false;
        } else {
            this.check1.checked = true;
        }
        if (this.entity.opts[2] == 0) {
            this.check2.checked = false;
        } else {
            this.check2.checked = true;
        }
        if (this.entity.opts[3] == 0) {
    ...
    BasicPropPanel.prototype.doClick = function (evt) {
        if (evt.target == this.check0) {
            var o = [];
            for (var i = 0; i < this.entity.opts.length; i++) {
                o.push(this.entity.opts[i]);
            }
            ...
            map[this.options.currowner.id].stack.execute(new FMValueChangedCmd(
                this.entity, "opts", o, this.options.currowner));
        } else if (evt.target == this.check1) {
            var o = [];
            for (var i = 0; i < this.entity.opts.length; i++) {
                o.push(this.entity.opts[i]);
            }
            ...
            map[this.options.currowner.id].stack.execute(new FMValueChangedCmd(
                this.entity, "opts", o, this.options.currowner));
        } else if (evt.target == this.check2) {
            var o = [];
            for (var i = 0; i < this.entity.opts.length; i++) {
                o.push(this.entity.opts[i]);
            }
            ...
            map[this.options.currowner.id].stack.execute(new FMValueChangedCmd(
                this.entity, "opts", o, this.options.currowner));
        } else if (evt.target == this.check3) {
            var o = [];
            for (var i = 0; i < this.entity.opts.length; i++) {
                o.push(this.entity.opts[i]);
            }
            if (this.check3.checked) {
                o[3] = 1;
            } else {
                o[3] = 0;
            }
