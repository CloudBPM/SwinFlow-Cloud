...
(function($, window, document, undefined) {
	var pluginName = "listSelectEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "list-input-props-nav-tabs";
		...
		var that = this;
		$("#list-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).listSelectPropPane({
			id : "tab0",
		...
		this.listSelectPropPane = plugin1.data("listSelectPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).listAssociatedPane({
			id : "tab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.listAssociatedPane = plugin2.data("listAssociatedPane");
		// by default
		$('#list-input-props-nav-tabs a[href="#tab' + this.currtabindex + '"]')
				.tab('show');
	...
	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.listAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.listAssociatedPane.removeRow(evt);
		}
	};
