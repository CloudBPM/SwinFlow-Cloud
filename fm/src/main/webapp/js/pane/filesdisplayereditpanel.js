...
(function($, window, document, undefined) {
	var pluginName = "filesDisplayerEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "fsd-input-props-nav-tabs";
		...
		var that = this;
		$("#fsd-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).filesDisplayerPropPane({
			id : "fsdtab0",
		...
		this.filesDisplayerPropPane = plugin1.data("filesDisplayerPropPane");
		...
		// by default
		$('#fsd-input-props-nav-tabs a[href="#fsdtab' + this.currtabindex + '"]').tab('show');
	...
	EditPanel.prototype.newTabHead = function(parent, index, caption, active) {
		...
		tabLink.setAttribute("href", "#fsdtab" + index);
		tabLink.setAttribute("aria-controls", "fsdtab" + index);
	...
	EditPanel.prototype.newTabContent = function(parent, index, active) {
		...
		tabContent.setAttribute("data", "fsdtab" + index);
		tabContent.setAttribute("role", "tabpanel");
		if (active)
			tabContent.className = "tab-pane active";
		else
			tabContent.className = "tab-pane";
		tabContent.id = "fsdtab" + index;
		return tabContent;
	};
