...
(function($, window, document, undefined) {
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "rdo-input-props-nav-tabs";
		...
		var that = this;
		$("#rdo-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).radioPropPane({
			id : "tab0",
		...
		this.radioPropPane = plugin1.data("radioPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).radioAssociatedPane({
			id : "tab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.radioAssociatedPane = plugin2.data("radioAssociatedPane");
		// by default
		$('#rdo-input-props-nav-tabs a[href="#tab' + this.currtabindex + '"]')
				.tab('show');
	...
	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.radioAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.radioAssociatedPane.removeRow(evt);
		}
	};
