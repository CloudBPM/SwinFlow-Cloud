...
(function($, window, document, undefined) {
	var pluginName = "integerInputEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "int-input-props-nav-tabs";
		...
		var that = this;
		$("#int-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).integerInputPropPane({
			id : "inttab0",
		...
		this.integerInputPropPane = plugin1.data("integerInputPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).integerInputAssociatedPane({
			id : "inttab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.integerInputAssociatedPane = plugin2.data("integerInputAssociatedPane");
		// by default
		$('#int-input-props-nav-tabs a[href="#inttab' + this.currtabindex + '"]').tab('show');
	...
	EditPanel.prototype.newTabHead = function(parent, index, caption, active) {
		...
		tabLink.setAttribute("href", "#inttab" + index);
		tabLink.setAttribute("aria-controls", "inttab" + index);
	...
	EditPanel.prototype.newTabContent = function(parent, index, active) {
		...
		tabContent.setAttribute("data", "inttab" + index);
		tabContent.setAttribute("role", "tabpanel");
		if (active)
			tabContent.className = "tab-pane active";
		else
			tabContent.className = "tab-pane";
		tabContent.id = "inttab" + index;
		return tabContent;
	};

	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.integerInputAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.integerInputAssociatedPane.removeRow(evt);
		}
	};
