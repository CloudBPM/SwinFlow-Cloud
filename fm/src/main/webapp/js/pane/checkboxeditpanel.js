...
(function($, window, document, undefined) {
	var pluginName = "checkboxEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "chk-input-props-nav-tabs";
		...
		var that = this;
		$("#chk-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).checkboxPropPane({
			id : "tab0",
		...
		this.checkboxPropPane = plugin1.data("checkboxPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).checkboxAssociatedPane({
			id : "tab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.checkboxAssociatedPane = plugin2.data("checkboxAssociatedPane");
		// by default
		$('#chk-input-props-nav-tabs a[href="#tab' + this.currtabindex + '"]')
				.tab('show');
	...
	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.checkboxAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.checkboxAssociatedPane.removeRow(evt);
		}
	};
