...
(function($, window, document, undefined) {
	var pluginName = "singleSelectEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "ssel-input-props-nav-tabs";
		...
		var that = this;
		$("#ssel-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).singleSelectPropPane({
			id : "tab0",
		...
		this.singleSelectPropPane = plugin1.data("singleSelectPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).singleSelectAssociatedPane({
			id : "tab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.singleSelectAssociatedPane = plugin2
				.data("singleSelectAssociatedPane");
		// by default
		$('#ssel-input-props-nav-tabs a[href="#tab' + this.currtabindex + '"]')
				.tab('show');
	...
	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.singleSelectAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.singleSelectAssociatedPane.removeRow(evt);
		}
	};
