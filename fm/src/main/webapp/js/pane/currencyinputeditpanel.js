...
(function($, window, document, undefined) {
	var pluginName = "currencyInputEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "curr-input-props-nav-tabs";
		...
		var that = this;
		$("#curr-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).currencyInputPropPane({
			id : "curtab0",
		...
		this.currencyInputPropPane = plugin1.data("currencyInputPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).currencyInputAssociatedPane({
			id : "curtab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.currencyInputAssociatedPane = plugin2.data("currencyInputAssociatedPane");
		// by default
		$('#curr-input-props-nav-tabs a[href="#curtab' + this.currtabindex + '"]').tab('show');
	...
	EditPanel.prototype.newTabHead = function(parent, index, caption, active) {
		...
		tabLink.setAttribute("href", "#curtab" + index);
		tabLink.setAttribute("aria-controls", "curtab" + index);
	...
	EditPanel.prototype.newTabContent = function(parent, index, active) {
		...
		tabContent.setAttribute("data", "curtab" + index);
		tabContent.setAttribute("role", "tabpanel");
		if (active)
			tabContent.className = "tab-pane active";
		else
			tabContent.className = "tab-pane";
		tabContent.id = "curtab" + index;
		return tabContent;
	};

	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.currencyInputAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.currencyInputAssociatedPane.removeRow(evt);
		}
	};
