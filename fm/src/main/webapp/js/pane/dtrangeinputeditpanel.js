...
(function($, window, document, undefined) {
	var pluginName = "dtrangeInputPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "dtr-input-props-nav-tabs";
		...
		var that = this;
		$("#dtr-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).dtrangeInputPropPane({
			id : "dtrtab0",
		...
		this.dtrangeInputPropPane = plugin1.data("dtrangeInputPropPane");

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		var plugin2 = $(this.tabContent2).dtrangeInputAssociatedPane({
			id : "dtrtab1",
			parent : this.options.parent,
			entity : entity,
			topparent : this.element,
		...
		this.dtrangeInputAssociatedPane = plugin2.data("dtrangeInputAssociatedPane");
		// by default
		$('#dtr-input-props-nav-tabs a[href="#dtrtab' + this.currtabindex + '"]').tab('show');
	...
	EditPanel.prototype.newTabHead = function(parent, index, caption, active) {
		...
		tabLink.setAttribute("href", "#dtrtab" + index);
		tabLink.setAttribute("aria-controls", "dtrtab" + index);
	...
	EditPanel.prototype.newTabContent = function(parent, index, active) {
		...
		tabContent.setAttribute("data", "dtrtab" + index);
		tabContent.setAttribute("role", "tabpanel");
		if (active)
			tabContent.className = "tab-pane active";
		else
			tabContent.className = "tab-pane";
		tabContent.id = "dtrtab" + index;
		return tabContent;
	};

	EditPanel.prototype.addRow = function(evt) {
		if (this.currtabindex == 1) {
			this.dtrangeInputAssociatedPane.addRow(evt);
		}
	};
	...
	EditPanel.prototype.removeRow = function(evt) {
		if (this.currtabindex == 1) {
			this.dtrangeInputAssociatedPane.removeRow(evt);
		}
	};
