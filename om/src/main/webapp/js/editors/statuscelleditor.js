...
(function($, window, document, undefined) {
	var pluginName = "orgStatusCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};

	var OrgStatusCellEditor = function(element, options) {
	...
	OrgStatusCellEditor.prototype.loadEditor = function(tag, entity, prop,
			isnull) {
		var that = this;
		if ($(tag).attr("type") == "h") {
			return;
		...
		for (x in yesno) {
			$("<option value='" + x + "'>" + validstatus[x] + "</option>")
					.appendTo($(selection));
		}
	...
	OrgStatusCellEditor.prototype.changeValue = function(tag, entity, prop,
			oldvalue, isnull) {
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$
						.data(this, pluginName, new OrgStatusCellEditor(this,
								options));
