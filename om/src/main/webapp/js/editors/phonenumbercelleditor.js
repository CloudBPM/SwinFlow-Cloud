...
(function($, window, document, undefined) {
	var pluginName = "phoneNumberCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};

	var PhoneNumberCellEditor = function(element, options) {
	...
	PhoneNumberCellEditor.prototype.loadEditor = function(tag, entity, prop,
			isnull) {
		var that = this;
		if ($(tag).attr("type") == "h") {
			return;
		...
		// var inputObj = $("<input type='tel' class='form-control'
		// id='inputPhoneNumber' placeholder='10 88886666'
		// required='required'>");
		// inputObj.val(oldvalue).appendTo($(tag)).get(0).select();
		//		
	...
	PhoneNumberCellEditor.prototype.changeValue = function(tag, entity, prop,
			oldvalue, isnull) {
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new PhoneNumberCellEditor(this,
						options));
