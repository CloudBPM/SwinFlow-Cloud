...
(function($, window, document, undefined) {
	var pluginName = "homePageValueEditor";
	var defaults = {
		parent : "",
		msg : "",
	};
	...
	TextCellEditor.prototype.loadEditor = function(tag, entity, prop,
			isnull) {
		var that = this;
		if ($(tag).attr("type") == "h") {
			return;
	...
	TextCellEditor.prototype.changeValue = function(tag, entity, prop,
		...
		if (isnull == "n" && (newvalue == null || newvalue == "")) {
			...
			if (newvalue != oldvalue) {
				for (x in entity) {
					if (x == prop) {
						map[entity.parent].stack.execute(new HomePageValueChangeCmd(entity, prop,
											newvalue));
						break;
					}
				}
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new TextCellEditor(this,
						options));
