...
(function($, window, document, undefined) {
	var pluginName = "authGroupTextAreaPropCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};

	var GroupTextAreaPropCellEditor = function(element, options) {
	...
	GroupTextAreaPropCellEditor.prototype.loadEditor = function(tabid, tag,
			entity, prop, isnull) {
		var that = this;
		this.tabId = tabid;
		if ($(tag).attr("type") == "h") {
	...
	GroupTextAreaPropCellEditor.prototype.changeValue = function(tag, entity,
		...
		if (isnull == "n" && (newvalue == null || newvalue == "")) {
			...
			if (newvalue != oldvalue) {
				map[this.tabId].stack.execute(new GroupPropsChangeCmd(
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new GroupTextAreaPropCellEditor(this,
						options));
