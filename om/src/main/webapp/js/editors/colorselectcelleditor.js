...
(function ($, window, document, undefined) {
    var pluginName = "colorSelectCellEditor";
    var defaults = {
        parent: "",
        msg: "",
    };
    ...
    TextCellEditor.prototype.loadEditor = function (tag, entity, prop, isnull, owner) {
        var that = this;
        if ($(tag).attr("type") == "h") {
            return;
        }
    ...
    TextCellEditor.prototype.changeValue = function (tag,
                                                     entity,
                                                     prop,
                                                     oldvalue,
                                                     isnull, owner) {
        ...
        if (isnull == "n" && (newvalue == null || newvalue == "")) {
            ...
            if (newvalue != oldvalue) {
                for (x in entity) {
                    if (x == prop) {
                        map[owner].stack.execute(
                            new OMMbPropChangedCmd(entity, prop, newvalue, owner));
                        break;
                    }
                }
                $(tag).text(newvalue);
