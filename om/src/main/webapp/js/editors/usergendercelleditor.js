...
(function($, window, document, undefined) {
	var pluginName = "userGenderCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};

	var UserGenderSelectCellEditor = function(element, options) {
	...
	UserGenderSelectCellEditor.prototype.loadEditor = function(tabid, tag,
			entity, prop, isnull) {
		var that = this;
		this.tabId = tabid;
		if ($(tag).attr("type") == "h") {
		...
		for (x in gender) {
			$("<option value='" + x + "'>" + gender[x] + "</option>").appendTo(
					$(sel));
		}
		if (entity instanceof Staff) {
			sel.val(entity.user.gender);
		} else if (entity instanceof User) {
			sel.val(entity.gender);
		}
		sel.focus();
	...
	UserGenderSelectCellEditor.prototype.changeValue = function(tag, entity,
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new UserGenderSelectCellEditor(this,
						options));
