...
(function($, window, document, undefined) {
	var pluginName = "userIdNumCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};

	var UserIDNumPropCellEditor = function(element, options) {
	...
	UserIDNumPropCellEditor.prototype.loadEditor = function(tabid, tag, entity,
			prop, isnull) {
		var that = this;
		this.tabId = tabid;
		if ($(tag).attr("type") == "h") {
				...
				.keydown(
						function(event) {
							...
							if (keyvalue == 9) { // Tab key
								...
								if (that.changeValue(tag, entity, prop,
										oldvalue, isnull)) {
									inputObj.remove();
									if (nextTD != undefined) {
									...
									&& keyvalue != 36
									&& keyvalue != 35
									&& !((keyvalue >= 48 && keyvalue <= 57) || (keyvalue >= 65 && keyvalue <= 90))) {
								return false;
							}
						})
				.keyup(
	...
	UserIDNumPropCellEditor.prototype.changeValue = function(tag, entity, prop,
			oldvalue, isnull) {
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new UserIDNumPropCellEditor(this,
						options));
