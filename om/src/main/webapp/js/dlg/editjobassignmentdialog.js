...
(function($, window, document, undefined) {
	var pluginName = "editJobAssignmentDialog";
	var defaults = {
		id : "",
		title : "",
	...
	var EditJobAssignmentDialog = function(element, options) {
	...
	EditJobAssignmentDialog.prototype.init = function(options) {
	...
	EditJobAssignmentDialog.prototype.loadPanel = function(parent) {
		...
		this.leftSelect.addEventListener('click', this, false);
		...
		this.rightSelect.addEventListener('click', this, false);
		...
		group3.setAttribute("role", "search");
		group3.setAttribute("aria-label", "");

		var descriptionDiv = document.createElement("DIV");
		group3.appendChild(descriptionDiv);
		descriptionDiv.className = "col-lg-12 col-md-12 col-sm-12 col-xs-12";

		this.description = document.createElement("textarea");
		descriptionDiv.appendChild(this.description);
		this.description.className = "form-control";
		this.description.setAttribute("placeholder", "职员列表变动原因描述...");

		this.loading(this.entity);
	};

	EditJobAssignmentDialog.prototype.crateSGroup = function(parent) {
	...
	EditJobAssignmentDialog.prototype.createTool = function(group, id, title,
	...
	// create <<, <, >, >> buttons
	EditJobAssignmentDialog.prototype.createMiddleButtons = function(listDiv) {
	...
	EditJobAssignmentDialog.prototype.loading = function(entity) {
		$("#progressbar").show();
		var that = this;
		$.getJSON(omservices.api(24, this.options.ownerId), {
			prid : entity.id,
			owner : entity.owner,
		}).complete(
				function(data) {
	...
	EditJobAssignmentDialog.prototype.loadData = function(data) {
		...
		this.editList = new JobAssignmentEditList();
		if (this.entity instanceof ProjectRole) {
			this.editList.source = 1;
		} else if (this.entity instanceof Position) {
			this.editList.source = 0;
		}
	...
	EditJobAssignmentDialog.prototype.setButtonStatus = function() {
	...
	EditJobAssignmentDialog.prototype.addOptions = function(parent, title,
			value, index) {
	...
	EditJobAssignmentDialog.prototype.show = function() {
	...
	EditJobAssignmentDialog.prototype.hide = function() {
		$(this.modalframe).modal('hide');
	};

	EditJobAssignmentDialog.prototype.handleEvent = function(e) {
		switch (e.type) {
		case "click":
			this.doClick(e);
			break;
	...
	EditJobAssignmentDialog.prototype.doKeydown = function(evt) {
	...
	EditJobAssignmentDialog.prototype.searchLeftValue = function(value) {
		if (value != "") {
			var lst = this.editList.searchLeftList(value);
			for (var j = 0; j < lst.length; j++) {
				var gm = this.editList.findMember(lst[j].staffId);
				var cd = "无";
				if (gm.staffCode != null && gm.staffCode != ""
						&& gm.staffCode != "null") {
					cd = gm.staffCode;
				}
				var n = gm.staffFullName + "(" + cd + ")";
	...
	EditJobAssignmentDialog.prototype.searchRightValue = function(value) {
		if (value != "") {
	...
	EditJobAssignmentDialog.prototype.doClick = function(evt) {
		if (evt.target.tagName == "OPTION") {
			if (evt.target.parentElement == this.leftSelect) {
				if (this.leftSelect.selectedIndex > -1) {
					this.setButtonStatus();
				}
			} else if (evt.target.parentElement == this.rightSelect) {
				if (this.rightSelect.selectedIndex > -1) {
					this.setButtonStatus();
				}
			}
		} else if (evt.target == this.removeAllButton
			...
			for (var j = 0; j < ids.length; j++) {
				...
				var n = this.createOptionTitle(gm);
				this.addOptions(this.rightSelect, n, gm.staffId, 0);
				this.editList.removedStaffIds.push(gm.staffId);
			}
			this.setButtonStatus();
			...
			for (var j = 0; j < ids.length; j++) {
				...
				var n = this.createOptionTitle(gm);
				this.addOptions(this.leftSelect, n, gm.staffId, 0);
				this.editList.addedStaffIds.push(gm.staffId);
			}
			this.setButtonStatus();
			this.enableButton(this.saveButton);
			...
			this.editList.description = Utils.stringify(this.description.value
					.trim());
			// console.log(this.editList);
			var that = this;
			$("#progressbar").show();
			$.post(omservices.api(25, this.options.ownerId), {
	...
	EditJobAssignmentDialog.prototype.createOptionTitle = function(obj) {
		var cd = "无";
	...
	EditJobAssignmentDialog.prototype.enableButton = function(button) {
		button.disabled = false;
	};

	EditJobAssignmentDialog.prototype.disableButton = function(button) {
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new EditJobAssignmentDialog(this,
						options));
