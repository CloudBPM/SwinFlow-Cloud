...
(function($, window, document, undefined) {
	var pluginName = "microServiceStoreDialog";
	var defaults = {
		id : "",
		title : "",
		...
		for (var i = 0; i < json.pageEntities.length; i++) {
			if (json.pageEntities[i] != null) {
				...
				if (json.pageEntities[i].appType == "1") {
					...
					appRadio.setAttribute("apptype", "1");
	...
	AppServiceStoreDialog.prototype.show = function(source) {
		this.source = source;
	...
	AppServiceStoreDialog.prototype.doClick = function(evt) {
		...
		if (evt.target == this.saveButton
			...
			var t = $("input[name=app]:checked").attr("apptype"); // 1: micro service; 2:java(not use)
			var v = $("input[name=app]:checked").val();// micro service ID
			var ap = $("input[name=app]:checked").attr("appid");// micro service ID
			var pw = $("input[name=app]:checked").attr("pw");// security key(password)
			var apn = $("input[name=app]:checked").attr("appname");// micro service name
			this.options.parent.setAppSearchResult(apn, t, v, ap, pw, this.source);
			this.hide();
			return;
