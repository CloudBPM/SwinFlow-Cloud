...
(function($, window, document, undefined) {
	var pluginName = "omOrganizationEditor";
	var defaults = {
		id : "",
		ownerId : "",
		basicpropsheet : "",
	...
	Editor.prototype.init = function(options) {
		...
		organization.id = options.id;
		this.currOwner = organization;
		this.basicpropsheet = options.basicpropsheet;
		this.propsheet = options.propsheet;
	...
	Editor.prototype.loading = function(options) {
		var that = this;
		$("#progressbar").show();
		var getting = $.getJSON(omservices.api(3, options.id), {
			id : options.id
		}).complete(
				function(data) {
					...
					// 		return;
					// 	}
						that.currOwner.parseFromJSON(data);
						that.repaint();
	...
	Editor.prototype.createToolbar = function(options) {
		...
		this.showallbutton = this.createTool(g3, "showall" + this.options.id,
		...
		this.printbutton = this.createTool(g4, "print" + this.options.id, "打印",
				"btn btn-default", "i", "fa fa-print fa-lg");
	...
	Editor.prototype.pallet = function(options) {
		...
		taskbar1.appendChild(this.createDepartmentationOnBar("一般部门", "e",
				this.currOwner.id, "img/department_32x32.png"));
		taskbar1.appendChild(this.createDepartmentationOnBar(
				"大部门/事业部/分支机构/办事处/营业部", "d", this.currOwner.id,
				"img/division_32x32.png"));
		taskbar1.appendChild(this.createDepartmentationOnBar("组/团队/项目组/研发组",
				"f", this.currOwner.id, "img/projectteam_32x32.png"));

	};

	Editor.prototype.createDepartmentationOnBar = function(dptname, type,
			imgid, iconsrc) {
	...
	Editor.prototype.createIcon = function(taskname, type, imgid, iconsrc,
		...
		icon.addEventListener('click', function(e) {
			...
			parent.editorStatus = 3; // draw department
		});
			...
			if (this.getMouseButton(evt) == "LEFT") { // drag
				if (this.editorStatus == 0) {
					...
					if (this.mouse0 != null) {
						this.mouse1 = this.getPointOnCanvas(evt);
						if (this.creating == "d") {
							this.tmpdepart = new Division();
							this.tmpdepart.name = Utils.stringify("大部门"
									+ this.currOwner.generateNewNumber());
						} else if (this.creating == "e") {
							this.tmpdepart = new Department();
							this.tmpdepart.name = Utils.stringify("一般部门"
									+ this.currOwner.generateNewNumber());
						} else if (this.creating == "f") {
							this.tmpdepart = new ProjectTeam();
							this.tmpdepart.name = Utils.stringify("组"
									+ this.currOwner.generateNewNumber());
						}
	...
	Editor.prototype.addtoGraph = function(part) {
		this.stack.execute(new OMCreateDepartmentCmd(part, this.selected,
				this.currOwner));
		this.creating = null;
		this.editorStatus = 1
	};
	...
	Editor.prototype.drawDepartment = function(tmpdepart) {
		var part = null;
		if (tmpdepart instanceof Department) {
			part = new Department();
		} else if (tmpdepart instanceof Division) {
			part = new Division();
		} else if (tmpdepart instanceof ProjectTeam) {
			part = new ProjectTeam();
		}
	...
	Editor.prototype.addDepartment = function(mouse) {
		...
		if (this.creating == "d") {
			part = new Division();
			part.name = Utils.stringify("大部门"
					+ this.currOwner.generateNewNumber());
		} else if (this.creating == "e") {
			part = new Department();
			part.name = Utils.stringify("一般部门"
					+ this.currOwner.generateNewNumber());
		} else if (this.creating == "f") {
			part = new ProjectTeam();
			part.name = Utils.stringify("组"
					+ this.currOwner.generateNewNumber());
		}
