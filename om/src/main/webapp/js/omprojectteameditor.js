...
(function($, window, document, undefined) {
	var pluginName = "omProjectTeamEditor";
	var defaults = {
		id : "",
		ownerId : "",
		basicpropsheet : "",
	...
	Editor.prototype.loading = function(options) {
		var that = this;
		$("#progressbar").show();
		$.getJSON(omservices.api(6, this.options.ownerId), {
			id : options.id
		}).complete(
				function(data) {
					...
					// 		return;
					// 	}
						that.currOwner.parseStructureFromJSON(data, options.ownerId);
						that.repaint();
	...
	Editor.prototype.createToolbar = function(options) {
		...
		this.lbutton = this.createTool(g3, "lalign" + this.options.id, "向左对齐",
				c, "span", "glyphicon glyphicon-object-align-left fa-lg");
		this.cbutton = this.createTool(g3, "valign" + this.options.id, "垂直对齐",
				c, "span", "glyphicon glyphicon-object-align-vertical fa-lg");
		this.rbutton = this.createTool(g3, "ralign" + this.options.id, "向右对齐",
				c, "span", "glyphicon glyphicon-object-align-right fa-lg");

		this.tbutton = this.createTool(g3, "talign" + this.options.id, "顶部对齐",
				c, "span", "glyphicon glyphicon-object-align-top fa-lg");
		this.mbutton = this.createTool(g3, "malign" + this.options.id, "中间对齐",
				c, "span", "glyphicon glyphicon-object-align-horizontal fa-lg");
		this.bbutton = this.createTool(g3, "balign" + this.options.id, "底部对齐",
		...
		this.showallbutton = this.createTool(g5, "showall" + this.options.id,
	...
	Editor.prototype.pallet = function(options) {
		...
		taskbar1.appendChild(this.createComponentOnBar("普通角色", "e",
				this.currOwner.id, "img/projectrole_32x32.png"));
			...
			if (this.getMouseButton(evt) == "LEFT") { // drag
				if (this.editorStatus == 0) {
					...
					if (this.mouse0 != null) {
						this.mouse1 = this.getPointOnCanvas(evt);
						if (this.creating == "e") {
							this.tmpdepart = new ProjectRole();
							this.tmpdepart.name = Utils.stringify("普通角色"
	...
	Editor.prototype.drawDepartment = function(tmpdepart) {
		var part = null;
		if (tmpdepart instanceof ProjectRole) {
			part = new ProjectRole();
		}
	...
	Editor.prototype.addDepartment = function(mouse) {
		...
		if (this.creating == "e") {
			part = new ProjectRole();
			part.name = Utils.stringify("普通角色"
					+ this.currOwner.generateNewNumber());
		}
