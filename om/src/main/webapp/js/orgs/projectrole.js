...
function ProjectRole() {
	...
	this.name = "project role";
	this.abbrName = "role abbr"; // short name
	...
	this.children = []; // 
	this.classtypename = "ProjectRole";
};

ProjectRole.prototype = new AbstractPosition();

ProjectRole.prototype.addChild = function(position) {
	this.children.push(position);
};

ProjectRole.prototype.clone = function() {
	var d = new Position();
	d.id = this.id;
...
ProjectRole.prototype.setParent = function(parent) {
	this.parent = parent;
...
ProjectRole.prototype.removeChild = function(child) {
...
ProjectRole.prototype.setSelected = function(selected) {
...
ProjectRole.prototype.drawToContext = function() {
...
ProjectRole.prototype.isInMark = function(x, y) {
...
ProjectRole.prototype.parseFromJSON = function(json, parent, currOwner, ownerid) {
	this.id = json.id;
	...
	this.context = currOwner.context;
	this.currOwner = currOwner.id;
	this.owner = ownerid;
	...
	if (json.children.length > 0) {
		for (var i = 0; i < json.children.length; i++) {
			...
			p.parseFromJSON(json.children[i], this, currOwner, ownerid);
			this.addChild(p);
		}
	}
};

ProjectRole.prototype.stringifyforJSON = function() {
	var p = new ProjectRole();
	p.id = this.id;
