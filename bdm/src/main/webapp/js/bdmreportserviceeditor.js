...
(function ($, window, document, undefined) {
    var pluginName = "reportServiceEditor";
    var defaults = {
        id: "", // process ID
        userId: "",
        userfullname: "",
    ...
    Editor.prototype.shading = function (options, currObject) {
        ...
        $("#rpt-props-nav-tabs" + options.id).on("click", "a", function (e) {
            ...
            if (that.currtabindex == 2) {
                if (that.queryVersionRangePane != undefined &&
                    that.queryVersionRangePane.initialize != undefined) {
                    that.queryVersionRangePane.initialize();
                }
            } else if (that.currtabindex == 3) {
                if (that.reportServicePreview.initialize != undefined) {
                    that.reportServicePreview.initialize();
                }
            }
        });
        ...
        if ($(this.tabContent2).queryConditionPane != undefined) {
            ...
            this.queryConditionPane = plugin1.data("queryConditionPane");
        }
        ...
        if ($(this.tabContent4).reportServicePreview != undefined) {
            var plugin1 = $(this.tabContent4).reportServicePreview({
    ...
    Editor.prototype.doClick = function (evt) {
        if (evt.target == this.undobutton
            ...
            if (this.reportServicePreview.refresh != undefined) {
                this.reportServicePreview.generate();
            }
        } else if (evt.target == this.pbutton
            || evt.target.id == ("preview" + this.options.id)) {
            service.previewreport(this.currObject.id);
        } else if (evt.target == this.refreshHButton
            || (evt.target.id == ("refreshS" + this.options.id))) {
            if (this.reportServicePreview.refresh != undefined) {
                this.reportServicePreview.refresh();
            }
