...
(function($, window, document, undefined) {
	...
	UploadPlugin.prototype.doClick = function(evt) {
		if (evt.target == this.iconC) {
			if (this.v.value.id != null && this.v.value.id != "") {
				this.messageDialog.show("该删除不可恢复，您确定要删除吗？");
			} else {
				if (this.uploadAction.completed == 0) {
					this.uploadAction.doCancel();
				} else if (this.uploadAction.completed == 1) {
					this.messageDialog.show("该删除不可恢复，您确定要删除吗？");
				}
			}
			...
			$("#progressbar").show();
			var fd = new FormData();
			fd.append('oid', this.v.owner);
			fd.append('path', this.v.value.owner + "/pm/"
					+ this.v.value.currOwner + "/" + this.v.value.id + "_"
					+ this.v.value.name);
			fd.append('fname', this.v.value.name);
			var xhh = new XMLHttpRequest();
			xhh.open("post", service.api2(3, this.v.owner), true);
