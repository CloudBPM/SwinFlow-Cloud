...
(function($, window, document, undefined) {
    ...
    Editor.prototype.createTopToolbar = function(options, toolbar) {
        ...
        this.tSearchInput.setAttribute("placeholder", "搜索音频名等");
    ...
    Editor.prototype.doClick = function(evt) {
        if ((evt.target.id == ("audio"+this.options.id))) {
            var url = evt.target.parentElement.getAttribute("path");
            // this.bookEditor.show(url);
            window.open("/file"+url);  //虚拟路径
            // window.open(nginxHost+url); //nignx路径
    ...
    Editor.prototype.loadData = function(obj) {
        ...
        if (objs != null && objs.length > 0) {
            for (var i = 0; i < objs.length; i++) {
                ...
                this.createCell(0, audio.audioName, row);
                this.createCell(1, audio.createTime,row);
                this.createCell(2, audio.sellType, row);
                this.createCell(3, audio.goodPrise+"元", row);
                this.createCell(4, audio.discountPrise+"元",row);
                // this.createCell(6, book.createTime,row);
                var cell = this.createCell(5, null, row);
                cell.setAttribute("path",audio.audioPath);
                var b0 = this.createIcon(cell, "audio"+this.options.id, "4", "fa fa-file-o fa-lg",
                    "view", "查看", "btn-primary");
