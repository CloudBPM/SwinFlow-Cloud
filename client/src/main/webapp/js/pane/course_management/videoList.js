...
(function($, window, document, undefined) {
    ...
    var Editor = function(element, options) {
        ...
        this.deprecated = 99;
        this.init(options);
        this.loading(1,15,"",this.options.userId)
    };
    ...
    Editor.prototype.createTopToolbar = function(options, toolbar) {
        ...
        this.tSearchInput.setAttribute("placeholder", "搜索视频名等");
    ...
    Editor.prototype.doClick = function(evt) {
        if ((evt.target.id == ("video"+this.options.id))) {
            var url = evt.target.parentElement.getAttribute("path");
            // this.bookEditor.show(url);
            url = "/file"+url;
            window.open(url, "_blank");
                // window.open(nginxHost+url, "_blank"); //nginx路径
    ...
    Editor.prototype.loadData = function(obj) {
        ...
        if (objs != null && objs.length > 0) {
            for (var i = 0; i < objs.length; i++) {
                ...
                var video = new Video();
                video.parseFromJSON(objs[i]);
                row.addEventListener('dblclick', this, false);
                row.addEventListener('click', this, false);
                row.setAttribute("id", video.id);
                this.createCell(0, video.videoName, row);
                this.createCell(1, video.createTime, row);
                this.createCell(2, video.sellType, row);
                this.createCell(3, video.goodPrise+"元", row);
                this.createCell(4, video.discountPrise+"元", row);
                // this.createCell(5,video.createTime,row);
                var cell = this.createCell(5, null, row);
                cell.setAttribute("path",video.videoPath);
                var b0 = this.createIcon(cell, "video"+this.options.id, "4", "fa fa-file-o fa-lg",
                    "view", "查看", "btn-primary");
