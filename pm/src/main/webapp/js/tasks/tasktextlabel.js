...
function TaskTextLabel(x0, y0, x1, y1, context) {
	this.x0 = x0;
	this.y0 = y0;
	this.x1 = x1;
	this.y1 = y1;
...
TaskTextLabel.prototype.outputText = function(text, alignment) {
	var maxWidth = this.x1 - this.x0;
	var x = Math.floor(this.x0) + 0.5;

	var content = "";
	if (text != null)
		content = text;
	var rows = Utils.getY(this.context, content, x, maxWidth, this.lineHeight);
	var y = Math.floor(this.y0 + 22) + 0.5;
	// var y = Math.floor((this.y0 + this.y1)/2 - (rows * lineHeight)/2)+0.5;

	this.context.font = '12pt Calibri'; // '16pt Calibri';
