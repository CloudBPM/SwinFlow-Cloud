...
(function($, window, document, undefined) {
	var pluginName = "startPointEditPanel";
	var defaults = {
		id : "",
		parent : "",
		entity : "",
	...
	EditPanel.prototype.loadPane = function(entity, tabindex, owner) {
		...
		tabUL.id = "stdp-input-props-nav-tabs";
		tabUL.setAttribute("role", "tablist");
		tabDIV.appendChild(tabUL);
		this.newTabHead(tabUL, 0, "可访问数据变量", true);
		this.newTabHead(tabUL, 1, "启动界面设定", false);
		var that = this;
		$("#stdp-input-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
		...
		var plugin1 = $(this.tabContent1).accessibleVariablesPane({
			...
			currowner : owner,// process object
		});
		...
		$('#stdp-input-props-nav-tabs a[href="#tab' + this.currtabindex + '"]')
				.tab('show');
		if (owner instanceof WfProcess) {
