...
(function($, window, document, undefined) {
	var pluginName = "formServiceStoreDialog";
	var defaults = {
		id : "",
		title : "",
	...
	ServiceStoreDialog.prototype.loadPanel = function(parent) {
		...
		this.searchInput.setAttribute("placeholder", "在表单商店可用的表单界面...");
	...
	ServiceStoreDialog.prototype.createTool = function(group, id, title, style,
			fonttag, fontclass) {
	...
	ServiceStoreDialog.prototype.doKeydown = function(evt) {
		...
		if (key == 13) {
			evt.preventDefault();
			if (evt.target.value != "") {
				this.searching(this.fid, evt.target.value, this.owner, 0, 30);
			} else {
	...
	ServiceStoreDialog.prototype.searching = function(id, condition, process,
			pageno, pagesize) {
		$("#progressbar").show();
		var that = this;
		$.getJSON(service.api(24), {
			...
			ownerid : process.owner,// organization id
			pageno : pageno,
			pagesize : pagesize,
	...
	ServiceStoreDialog.prototype.loadData = function(json) {
		...
		if (json.pageEntities != null && json.pageEntities.length == 0) {
			...
			entiyDiv.innerHTML = "抱歉，没有搜索到符合条件的表单界面。";
			return;
		}
		for (var i = 0; i < json.pageEntities.length; i++) {
			if (json.pageEntities[i] != null) {
				...
				appRadio.setAttribute("title", "请点击这里以选择该服务。");
				var that = this;
				...
				titleH3.innerHTML = json.pageEntities[i].frmName + " ";
				...
				devDiv.innerHTML = "开发：" + json.pageEntities[i].orgName;
	...
	ServiceStoreDialog.prototype.doClick = function(evt) {
		...
		if (evt.target == this.saveButton
			...
			this.options.parent.setNewForm($("input[name=frm]:checked").val());
			this.hide();
			return;
			...
			|| evt.target.id == "isearchFrmBtn" + this.options.id) {
			if (this.searchInput.value != "") {
				this.searching(this.fId, this.searchInput.value, this.owner, 0,
						30);
			} else {
				this.messageBox.show(4, "搜索对象不能为空，请输入一个搜索对象", false);
			}
