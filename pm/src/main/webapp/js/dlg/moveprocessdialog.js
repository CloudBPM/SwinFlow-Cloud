...
(function($, window, document, undefined) {
	var pluginName = "moveWfProcessDialog";
	var defaults = {
		id : "",
		title : "",
		parent : "",
	...
	var MoveProcessDialog = function(element, options) {
	...
	MoveProcessDialog.prototype.init = function(options) {
	...
	MoveProcessDialog.prototype.loadDialogBody = function(options, parent) {
	...
	MoveProcessDialog.prototype.handleEvent = function(e) {
		switch (e.type) {
		case "click":
			this.doClick(e);
			break;
	...
	MoveProcessDialog.prototype.show = function(source) {
		...
		var api = 26;
		if (arry[0] == "2") { // folder object
			if (arry[2] == "109") {
				api = 11;
			} else if (arry[2] == "100") {
				api = 26;
			}
		} else if (arry[0] == "3") { // process object
			if (arry[3] == "R") {
				api = 11;
			} else if (arry[3] == "P") {
				api = 26;
		...
		$(this.folderContentDiv1).jstree({
			...
			if (arry[0] == "1") { // 1 means it is orgnization ID
				...
				if (source.parent == data.selected[0]) {
					that.saveButton.classList.add("disabled");
				} else {
	...
	MoveProcessDialog.prototype.doClick = function(evt) {
		if (evt.target.id == "OK" + this.options.id) {
			...
			if (selected.length > 0) {
				...
				if (arry[0] == "1") {
					...
					this.options.parent.moveObjcets(t);
				}
			} else {
				this.messageBox.show(4, "请选择目标目录", false);
				return;
			}
	...
	MoveProcessDialog.prototype.hide = function() {
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new MoveProcessDialog(this, options));
