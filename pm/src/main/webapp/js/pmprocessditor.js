...
(function ($, window, document, undefined) {
    var pluginName = "wfProcessEditor";
    var defaults = {
        id: "",
        basicpropsheet: "",
        propsheet: "",
    ...
    Editor.prototype.loading = function (options) {
        var that = this;
        $("#progressbar").show();
        $.getJSON(service.api(6, options.owner), {
            id: options.id,
            owner: options.owner,
        }).complete(function (data) {
            if (data.responseJSON.status != -5) {
                that.wfprocess.parseFromJSON(data.responseJSON);
                that.initPallet(options);
                that.initCanvas(options);
    ...
    Editor.prototype.initMainPanel = function (options) {
        ...
        var plugin = $(this.element).releaseWfProcessDialog({
            id: "pm093",
            title: "轩琦科技 - 创建新版本",
        ...
        this.releaseWfProcessDialog = plugin.data("releaseWfProcessDialog");
    };
    ...
    Editor.prototype.createToolbar = function (options) {
        ...
        this.sbutton = this.createTool(g5, "simulation" + this.options.id,
            "仿真执行", "btn btn-default", "i", "fa fa-bug fa-lg");

        var g6 = this.createGroup(parent);
        this.pbutton = this.createTool(g6, "publish" + this.options.id,
            "创建新版本", c, "i", "fa fa-globe fa-lg");
    ...
    Editor.prototype.initPallet = function (options) {
        ...
        var taskbar1 = this.createPalletBar("One" + options.id, "基本", true, "in", group);
        var taskbar2 = this.createPalletBar("Two" + options.id, "高级", false, "", group);
        var taskbar3 = this.createPalletBar("Three" + options.id, "定制", false, "", group);

        taskbar1.appendChild(this.component("连接", options.id, "tr")); //
        taskbar1.appendChild(this.component("起点", options.id, "st"));
        taskbar1.appendChild(this.component("终点", options.id, "ed"));
        if (this.wfprocess.workflowType > 0) {
            taskbar1.appendChild(this.component("人工参与任务", options.id, "man"));
        }
        taskbar1.appendChild(this.component("系统自动任务", options.id, "sys"));
        taskbar1.appendChild(this.component("赋值任务", options.id, "ass"));

        taskbar2.appendChild(this.component("延迟等待任务", options.id, "wait"));
        ...
        taskbar2.appendChild(this.component("子应用调用", options.id, "sub"));
    ...
    Editor.prototype.initCanvas = function (options) {
        ...
        //this.wfprocess.context = this.context;
        //this.repaint();
    ...
    Editor.prototype.doClick = function (evt) {
        if (evt.target == this.undobutton
            ...
            || evt.target.id == ("simulation" + this.options.id)) {
            console.log("13");
        } else if (evt.target == this.pbutton
            || evt.target.id == ("publish" + this.options.id)) {
            this.releaseWfProcessDialog.setWfProcess(this.wfprocess);
            this.releaseWfProcessDialog.show();
        }
    };
    ...
    Editor.prototype.doCreationAction = function (obj) {
        var that = this;
        $.getJSON(service.api(2, this.options.owner)).complete(function (data) {
            if (obj != null) {
