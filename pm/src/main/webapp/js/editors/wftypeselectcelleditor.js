...
(function($, window, document, undefined) {
	var pluginName = "workflowTypeSelectCellEditor";
	var defaults = {
		parent : "",
		msg : "",
	};
	...
	SelectCellEditor.prototype.loadEditor = function(tag, entity, prop, isnull,
		...
		for (x in workflowtype) {
			$("<option value='" + x + "'>" + workflowtype[x] + "</option>")
					.appendTo($(selection));
		}
		selection.val(entity.workflowType);
		selection.focus();
	...
	SelectCellEditor.prototype.changeValue = function(tag, entity, prop,
		...
		if (isnull == "n" && (newvalue == null || newvalue == "-1")) {
			...
			if (newvalue != oldvalue) {
				map[this.owner.id].stack.execute(new PMValueChangedCmd(entity,
