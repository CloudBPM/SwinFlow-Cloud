...
(function($, window, document, undefined) {
	var pluginName = "pmRlWfProcessEditor";
	var defaults = {
		id : "",
		owner : "",
		userId : "",
	...
	Editor.prototype.createCells = function(obj) {
		var rwp = new ReleasedWfProcess();
		rwp.parseFromJSON(obj);
		this.listViewPane.objects.push(rwp);
		var row = this.listViewPane.tableList.insertRow(-1);
		row.setAttribute("key", rwp.id);
		row.addEventListener("click", this, false);
		row.addEventListener("dblclick", this, false);

		this.listViewPane.createCell(0, Utils.parse(rwp.name), row);
		this.listViewPane.createCell(1, workflowtype[rwp.workflowType], row);
		this.listViewPane.createCell(2, Utils.parse(rwp.author), row);
		this.listViewPane.createCell(3, Utils.parse(rwp.version), row);
		this.listViewPane.createCell(4, pfonlinestatus[rwp.deprecated], row);

		var cell = this.listViewPane.createCell(5, null, row);
		var b0 = this.createIcon(cell, rwp.id, "4", "fa fa-file-o fa-lg",
				"view", "查看", "btn-primary");
		var b1 = this.createIcon(cell, rwp.id, "0", "fa fa-cloud-upload fa-lg",
				"online", "上线", "");
		var b2 = this.createIcon(cell, rwp.id, "1",
				"fa fa-cloud-download fa-lg", "offline", "下线", "");
		var b3 = this.createIcon(cell, rwp.id, "2",
	...
	Editor.prototype.createHeaders = function(row) {
		this.listViewPane.createHead("应用名称", row);
		this.listViewPane.createHead("应用类型", row);
