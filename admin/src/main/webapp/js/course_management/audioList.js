...
(function($, window, document, undefined) {
    var pluginName = "audioListEditor";
    var defaults = {
        id : "",
        owner : "",
        userId : "",
    ...
    Editor.prototype.init = function(options) {
        ...
        var p4 = $(this.element).audioViewEditor({
            id : "002",
            title:"添加音频",
            owner : options.owner,
        ...
        this.audioEditor = p4.data("audioViewEditor")
    ...
    Editor.prototype.createSelection = function(parent) {
        var group = this.createGroup(parent);
        this.statusSelect = this.createTool(group, "addaudio" + this.options.id,
            "添加音频", "btn btn-default btn-success", "addaudio",
            "fa fa-plus fa-lg");
    ...
    Editor.prototype.createCells = function(obj) {
        var audio = new Audio();
        audio.parseFromJSON(obj);
        this.listViewPane.objects.push(audio);
        var row = this.listViewPane.tableList.insertRow(-1);
        row.setAttribute("key", audio.id);
        row.addEventListener("click", this, false);
        row.addEventListener("dblclick", this, false);

        this.listViewPane.createCell(0, audio.audioName, row);
        this.listViewPane.createCell(1, audio.createTime, row);
        this.listViewPane.createCell(2, audio.sellType, row);
        this.listViewPane.createCell(3, audio.goodPrise, row);
        this.listViewPane.createCell(4, audio.discountPrise, row);

        var cell = this.listViewPane.createCell(5, null, row);
        cell.setAttribute("key",audio.audioPath);
        var b0 = this.createIcon(cell, audio.id, "4", "fa fa-file-o fa-lg",
            "view", "查看", "btn-primary",audio);
        this.b1 = this.createIcon(cell, audio.id, "0", "fa fa-cloud-upload fa-lg",
            "online", "上线", "");
        this.b2 = this.createIcon(cell, audio.id, "1",
            "fa fa-cloud-download fa-lg", "offline", "下线", "");
        var b3 = this.createIcon(cell, audio.id, "2",
            "fa fa-thumbs-o-down fa-lg", "sendback", "审核未通过", "");
        var b4 = this.createIcon(cell, audio.id, "3",
    ...
    Editor.prototype.createIcon = function(parent, id, num, classname, name,
        ...
        button.addEventListener("click", function(evt) {
            if (this.name == "sendback") {
                ...
                that.audioEditor.show();
                that.audioEditor.loadDataFormDB(data);
            } else if (this.name == "online") {
                that.upperShelf(id)
            } else if (this.name == "offline") {
                that.lowerShelf(id)
            }else if (this.name == "delete"){
                that.deleteAudio(id,parent.getAttribute("key"));
            }
    ...
    Editor.prototype.deleteAudio = function(id,path){
        $("#progressbar").show();
        var that = this;
        $.post(service.api(35), {
            id : id,
    ...
    Editor.prototype.doClick = function(evt){
        if (evt.target == this.addNew || (evt.target.id == (  "addaudio" + this.options.id))) {
            this.audioEditor.loadData(new Audio(),true);
        }
    ...
    Editor.prototype.createHeaders = function(row) {
        this.listViewPane.createHead("音频名称", row);
