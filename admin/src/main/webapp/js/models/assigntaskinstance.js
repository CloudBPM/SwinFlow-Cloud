...
function AssignTaskInstance() {
	...
	this.classtypename = "AssignTaskInstance";
...
AssignTaskInstance.prototype = new AssignTask();

AssignTaskInstance.prototype.addAssignment = function(assignment) {
	this.assignments.push(assignment);
};

AssignTaskInstance.prototype.getAssignmentById = function(id) {
...
AssignTaskInstance.prototype.removeAssignment = function(assignment) {
	for (var i = 0; i < this.assignments.length; i++) {
...
AssignTaskInstance.prototype.setAssignment = function(assignment) {
	for (var i = 0; i < this.assignments.length; i++) {
		if (this.assignments[i].id == assignment.id
				|| this.assignments[i] == assignment) {
			this.assignments.splice(i, 1);
...
AssignTaskInstance.prototype.clone = function() {
	var a = new AssignTaskInstance();
	a.id = this.id;
	a.name = this.name;
...
AssignTaskInstance.prototype.stringifyforJSON = function() {
	var a = new AssignTaskInstance();
	a.id = this.id;
	a.name = this.name;
	a.x0 = this.x0; // top left corner X
...
AssignTaskInstance.prototype.parse = function(json) {
	this.id = json.id;
	this.name = json.name;
...
AssignTaskInstance.prototype.drawToContext = function() {
	...
	assignIcon48x48(this.status, this.context, this.x1 - 48, this.y1 - 48);
...
AssignTaskInstance.prototype.isInMark = function(x, y) {
