...
function SubprocessPointInstance() {
	...
	this.classtypename = "SubprocessPointInstance";
...
SubprocessPointInstance.prototype = new SubprocessPoint();

SubprocessPointInstance.prototype.clone = function() {
	var a = new SubprocessPointInstance();
	a.id = this.id;
	a.name = this.name;
...
SubprocessPointInstance.prototype.stringifyforJSON = function() {
	var a = new SubprocessPointInstance();
	a.id = this.id;
	a.name = this.name;
	a.x0 = this.x0; // top left corner X
	...
	a.subprocessInstanceId = this.subprocessInstanceId;
	for (var i = 0; i < this.subprocessInputs.length; i++) {
...
SubprocessPointInstance.prototype.getSubInputById = function(id) {
	for (var i = 0; i < this.subprocessInputs.length; i++) {
...
SubprocessPointInstance.prototype.addSubInput = function(assignment) {
	this.subprocessInputs.push(assignment);
};

SubprocessPointInstance.prototype.removeSubInput = function(assignment) {
	for (var i = 0; i < this.subprocessInputs.length; i++) {
...
SubprocessPointInstance.prototype.setSubInput = function(assignment) {
	for (var i = 0; i < this.subprocessInputs.length; i++) {
		if (this.subprocessInputs[i].id == assignment.id
				|| this.subprocessInputs[i] == assignment) {
			this.subprocessInputs.splice(i, 1);
...
SubprocessPointInstance.prototype.getSubOutputById = function(id) {
	for (var i = 0; i < this.subprocessOutputs.length; i++) {
...
SubprocessPointInstance.prototype.addSubOutput = function(assignment) {
	this.subprocessOutputs.push(assignment);
};

SubprocessPointInstance.prototype.removeSubOutput = function(assignment) {
	for (var i = 0; i < this.subprocessOutputs.length; i++) {
...
SubprocessPointInstance.prototype.setSubOutput = function(assignment) {
	for (var i = 0; i < this.subprocessOutputs.length; i++) {
		if (this.subprocessOutputs[i].id == assignment.id
				|| this.subprocessOutputs[i] == assignment) {
			this.subprocessOutputs.splice(i, 1);
...
// parse JSON string
SubprocessPointInstance.prototype.parse = function(json) {
	this.id = json.id;
	this.name = json.name;
	...
	this.subprocessInstanceId = json.subprocessInstanceId;
...
SubprocessPointInstance.prototype.drawToContext = function() {
	...
	subProcessIcon48x48(this.status, this.context, this.x1 - 48, this.y1 - 48);
...
SubprocessPointInstance.prototype.isInMark = function(x, y) {
