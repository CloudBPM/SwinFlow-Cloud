...
function ManualTaskInstance() {
	...
	this.classtypename = "ManualTaskInstance";
	...
	this.expiryDateTime = -1;
	this.alarmDateTime = -1;
	this.expiryHandlerProcessId;
	this.phase = -1;
	// 0: does not submitted; 1: submitted
	this.submitted = 0;
	this.submitterId = null;
	this.submitter = null;
	this.submitterIp = null;
	this.candidates = [];
	this.invitations = [];
};

ManualTaskInstance.prototype = new ManualTask();

ManualTaskInstance.prototype.clone = function() {
	var a = new ManualTaskInstance();
	a.id = this.id;
	a.name = this.name;
...
ManualTaskInstance.prototype.cloneAccessibleVarList = function() {
	var vars = [];
...
ManualTaskInstance.prototype.cloneParticipantList = function() {
	var parts = [];
...
ManualTaskInstance.prototype.parse = function(json) {
	this.id = json.id;
	this.name = json.name;
	...
	if (json.candidates != null && json.candidates.length > 0) {
		for (var i = 0; i < json.candidates.length; i++) {
			this.candidates.push(json.candidates[i]);
		}
	}
	// runtime props
	...
	this.expiryDateTime = json.expiryDateTime;
	this.alarmDateTime = json.alarmDateTime;
	this.expiryHandlerInstanceId = json.expiryHandlerInstanceId;
	// 0: does not submitted; 1: submitted
	this.submitted = json.submitted;
	this.submitterId = json.submitterId;
	this.submitter = json.submitter;
	this.submitterIp = json.submitterIp;
	this.phase = json.phase;
};

ManualTaskInstance.prototype.stringifyforJSON = function() {
	var a = new ManualTaskInstance();
	a.id = this.id;
	a.name = this.name;
	a.x0 = this.x0; // top left corner X
	...
	a.expiryDateTime = this.expiryDateTime;
	a.alarmDateTime = this.alarmDateTime;
	a.expiryHandlerInstanceId = this.expiryHandlerInstanceId;
	// 0: does not submitted; 1: submitted
	a.submitted = this.submitted;
	a.submitterId = this.submitterId;
	a.submitter = this.submitter;
	a.submitterIp = this.submitterIp;
	a.phase = this.phase;
	a.candidates = this.candidates;
	return a;
};

// A function for drawing the particle.
ManualTaskInstance.prototype.drawToContext = function() {
	...
	manualTaskIcon48x48(this.status, this.context, this.x1 - 48, this.y1 - 48);
...
ManualTaskInstance.prototype.fetchAccessibleVar = function(id) {
	if (this.accessibleVars != null && this.accessibleVars.length > 0) {
		for (var i = 0; i < this.accessibleVars.length; i++) {
...
ManualTaskInstance.prototype.isInMark = function(x, y) {
...
ManualTaskInstance.prototype.verify = function(owner) {
	return this.verifyAccessible(owner) && this.verifyBound(owner);
};

ManualTaskInstance.prototype.verifyAccessible = function(owner) {
	var f = false;
	if (this.accessibleVars != null && this.accessibleVars.length > 0) {
...
ManualTaskInstance.prototype.verifyBound = function(owner) {
	if (this.formContent == null) {
		return true;
	}
