...
function TransitionInstance() {
	...
	//DEFAULT = 0;ENABLED = 1;COMPLETED = 2;
	//UNUSED = 3;EXCEPTION = 4;
	this.status = 0;
	this.x0 = 0;
	this.y0 = 0;
	this.x1 = 0;
...
TransitionInstance.prototype = new Transition();

TransitionInstance.prototype.clone = function() {
	var a = new TransitionInstance();
	a.id = this.id;
	a.name = this.name;
	a.x0 = this.x0;
	a.y0 = this.y0;
	...
	// ignore business rule, keep always true.
	// a.alwaysTrue = this.alwaysTrue; 
...
TransitionInstance.prototype.parse = function(t, proc) {
	this.id = t.id;
	this.name = t.name;
	...
	this.definitionId = t.definitionId;
	this.updatePosition();
};

TransitionInstance.prototype.stringifyforJSON = function() {
	var t = new TransitionInstance();
	t.id = this.id;
	t.name = this.name;
	...
	t.definitionId = this.definitionId;
	return t;
};

// A function for drawing the particle.
TransitionInstance.prototype.drawToContext = function() {
	var tmp = this.context.strokeStyle;

	if (this.status == 0) {// default
		this.context.strokeStyle = "black";
		if ((this.source != null && this.source.isParallelOutput != undefined && this.source.isParallelOutput == true)
				|| (this.target != null
						&& this.target.isParallelInput != undefined && this.target.isParallelInput == true)) {
			this.context.strokeStyle = "blue";
		}
	} else {
		this.context.strokeStyle = Utils.toTransitionLineColor(this.status);
	}
...
TransitionInstance.prototype.isInMark = function(x, y) {
...
TransitionInstance.prototype.drawCurve = function(context, x0, y0, xm, ym, x1, y1) {
	// if (x0 < xm) {
...
TransitionInstance.prototype.updatePosition = function() {
	this.computeCoordinate();
	this.computeArrow();
};

// A function for drawing the particle.
TransitionInstance.prototype.computeCoordinate = function() {
 ...
 *            int
 */
TransitionInstance.prototype.computeArrow = function() {
