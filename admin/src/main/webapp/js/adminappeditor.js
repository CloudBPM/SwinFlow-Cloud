...
(function($, window, document, undefined) {
	...
	Editor.prototype.initCanvas = function(options) {
		...
		tabUL.className = "nav nav-tabs admin-nav-tabs";
		tabUL.id = "admin-props-nav-tabs";
		...
		if ($(this.tabContent1).searchAppPane != undefined) {
			var plugin1 = $(this.tabContent1).searchAppPane({
				id : "tab0",
				pid : options.id,
				uid : options.uid,
			...
			this.egnPane = plugin1.data("searchAppPane");
		}

		this.tabContent2 = this.newTabContent(tabContents, 1, false);
		if ($(this.tabContent2).overviewAppPane != undefined) {
			var plugin2 = $(this.tabContent2).overviewAppPane({
				id : "tab1",
				pid : options.id,
				basicpropsheet : options.basicpropsheet,
				propsheet : options.propsheet,
			...
			this.schPane = plugin2.data("overviewAppPane");
		}
		...
		var that = this;
		$("#admin-props-nav-tabs").on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
			that.currtabindex = parseInt($(this).attr('href').substring(4));
		});
		$('#admin-props-nav-tabs a[href="#tab' + this.currtabindex + '"]').tab(
				'show');
